...
# Added additions, deletions, bodyText, and reviews for accurate UI and comment counting
query = """
query {
  user(login: "%s") {
    pullRequests(states: MERGED, first: 100, orderBy: {field: CREATED_AT, direction: DESC}) {
...
def generate_card_string(pr_list):
    ...
    for pr in pr_list:
        ...
        # Build the Markdown "Card" UI
        content += f"### 🔀 [{title}]({url})\n"
        content += f"**[{repo}](https://github.com/{repo})** • 🌟 {stars} Stars\n\n"
        content += f"> {body}\n>\n"
        content += f"> 🟩 **+{additions}** 🟥 **-{deletions}** lines changed • 💬 **{total_comments} comments**\n\n"
        content += "<br>\n\n"
...
def format_markdown_output(prs):
    ...
    # Build the main section for the Top 5 using the new card layout
